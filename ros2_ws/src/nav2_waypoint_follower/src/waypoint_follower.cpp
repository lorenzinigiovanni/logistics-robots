    ...
    "nav2_core::WaypointTaskExecutor")
{
  ...
  declare_parameter("wait_time_if_equal", 5000);
...
WaypointFollower::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  wait_time_if_equal_ = get_parameter("wait_time_if_equal").as_int();
...
WaypointFollower::followWaypoints()
{
  ...
  while (rclcpp::ok()) {
    ...
    if (current_goal_status_ == ActionStatus::FAILED) {
      ...
      // Il nostro codice
      if(goal_index > 0 && goal->poses[goal_index - 1] == goal->poses[goal_index]) {
        RCLCPP_INFO(
          get_logger(),
          "Waypoint %i == %i, sleeping for %i milliseconds",
          goal_index - 1,
          goal_index,
          wait_time_if_equal_);
        rclcpp::sleep_for(std::chrono::milliseconds(wait_time_if_equal_));
      }
    }
